...
function initialize() {
    ...
    if (savedLocations) {
        ...
        //try to geolocate, otherwise set city to Jersey City
        if (!navigator.geolocation) {
            //can't geolocate and no previous searches, so just give them one
            getCurrent("Jersey City");
        } else {
...
function getCurrent(city) {
    var queryURL = "https://api.openweathermap.org/data/2.5/weather?q=" + city + "&APPID=3aa0185a32fa2f1fff5f5a7f8653499a&units=imperial";
    $.ajax({
        url: queryURL,
        method: "GET",
        error: function(err) {
        ...
        var uvURL = "https://api.openweathermap.org/data/2.5/uvi?lat=" + response.coord.lat + "&lon=" + response.coord.lat + "&APPID=3aa0185a32fa2f1fff5f5a7f8653499a&units=imperial";
        $.ajax({
            url: uvURL,
            method: "GET"
...
function getForecast(city) {
    console.log('we hit the 5 day forecast!!!')
        //get 5 day forecast
    var queryURL = "https://api.openweathermap.org/data/2.5/forecast?id=" + city + "&APPID=3aa0185a32fa2f1fff5f5a7f8653499a&units=imperial";
    $.ajax({
        url: queryURL,
        method: "GET"
    }).then(function(response) {
